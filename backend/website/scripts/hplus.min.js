function loading() {
    if (window.loadingId) {
        layer.close(window.loadingId);
    }
    window.loadingId = layer.load(2, {shade:[0.01, '#fff']});
}
...
function confirm(text, confirm) {
    layer.confirm(text, {btn: ['确定', '取消'], 'area': '310px'}, function(index){
        layer.close(index);
        confirm();
    });
}
function openWindow(options) {
    window.windowId = layer.open(options);
}
function closeCurrentWindow() {
    if (window.windowId) {
        layer.close(window.windowId);
    }
}
...
function SmoothlyMenu() {
    $("body").hasClass("mini-navbar") ? $("body").hasClass("fixed-sidebar") ? ($("#side-menu,.nav-header a").hide(), setTimeout(function () {
        $("#side-menu,.nav-header a").fadeIn(500)
    }, 300)) : $("#side-menu,.nav-header a").removeAttr("style") : ($("#side-menu,.nav-header a").hide(), setTimeout(function () {
        $("#side-menu,.nav-header a").fadeIn(500)
    }, 100))
}
function localStorageSupport() {
 ...
 $(document).ready(function () {
    ...
    }), $(".full-height-scroll").slimScroll({height: "100%"}), $("#side-menu>li,.navbar-open").click(function () {
