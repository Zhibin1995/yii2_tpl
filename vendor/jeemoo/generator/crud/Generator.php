<?php
namespace jeemoo\generator\crud;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $controllerBaseClass = 'backend\common\BaseController';
    ...
    public function getModelClass()
    {
        if (!empty($this->tableName)) {
            return Inflector::id2camel(str_replace('_', '-', $this->tableName));
        }
        return '';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'controllerNs', 'controllerBaseClass', 'tableName', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'], 'filter', 'filter' => 'trim'],
            [['ns', 'controllerNs', 'queryNs'], 'filter', 'filter' => function ($value) {
                return trim($value, '\\');
            }],

            [['db', 'ns', 'controllerNs', 'controllerBaseClass', 'tableName', 'baseClass', 'queryNs', 'queryBaseClass'], 'required'],
            [['db', 'queryClass'], 'match', 'pattern' => '/^\w+$/', 'message' => 'Only word characters are allowed.'],
            [['ns', 'controllerNs', 'controllerBaseClass', 'baseClass', 'queryNs', 'queryBaseClass'], 'match', 'pattern' => '/^[\w\\\\]+$/', 'message' => 'Only word characters and backslashes are allowed.'],
            [['tableName'], 'match', 'pattern' => '/^([\w ]+\.)?([\w\* ]+)$/', 'message' => 'Only word characters, and optionally spaces, an asterisk and/or a dot are allowed.'],
            [['db'], 'validateDb'],
            [['ns', 'controllerNs', 'queryNs'], 'validateNamespace'],
            [['tableName'], 'validateTableName'],
            [['baseClass'], 'validateClass', 'params' => ['extends' => ActiveRecord::className()]],
            [['controllerBaseClass'], 'validateClass', 'params' => ['extends' => Controller::className()]],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'controllerBaseClass' => 'Controller Base Class',
    ...
    {
        return array_merge(parent::stickyAttributes(), ['ns', 'db', 'baseClass','controllerNs','controllerBaseClass', 'generateRelations', 'generateLabelsFromComments', 'queryNs', 'queryBaseClass']);
    }
    ...
    public function generateScenarios($table)
    {
        ...
        foreach ($table->columns as $column) {
            if (!in_array($column->name, ['id', 'create_user', 'create_at', 'update_user', 'update_at'])) {
                $columns[] = $column->name;
            }
        }

        $scenarios['search'] = $columns;
        $scenarios['create'] = $columns;
        $scenarios['update'] = $columns;
        return $scenarios;
    }
    ...
    public function generateSearchConditions($tableSchema)
    {
        ...
        if (($table = $tableSchema) === false) {
            ...
            foreach ($table->columns as $column) {
                if ($column->type == 'string' && !strpos($column->name, 'img') && !in_array($column->name, ['img_url', 'summary'])) {
                    if (!empty($column->enumValues)) {
                        $columns[$column->name] = Schema::TYPE_INTEGER;
                    } else {
        ...
        foreach ($columns as $column => $type) {
            switch ($type) {
                ...
                case Schema::TYPE_TIMESTAMP:
                    $hashConditions[] = "'{$column}' => \$model->{$column},";
                    break;
                default:
                    $likeConditions[] = "\$query->andFilterWhere(['like', '{$column}', \$model->{$column}])";
                    break;
            }
        }
     ...
     * @return string
     */
    public function generateActiveField($tableSchema, $attribute, $relations = [])
    {
        ...
        if ($column->phpType === 'boolean') {
        ...
        } elseif ($column->type == 'string' && $column->size > 255) {
            return "\$form->field(\$model, '$attribute')->textarea(['rows' => 6])";
        } elseif ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute',['inputOptions'=>['class'=>''],])->textarea(['rows' => 6])";
        } else {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute')->dropDownList(" . $this->getModelClass() . "::get" . Inflector::pluralize(Inflector::id2camel($column->name)) . "(), ['prompt' => '请选择'])";
            } else {
                $model = false;
                foreach ($relations as $name => $relation) {
                    if ($relation[3] == $attribute) {
                        $model = $relation;
                        break;
                    }
                }
                if ($model) {
                    return "\$form->field(\$model, '$attribute')->dropDownList(" . $model[1] . "::getSelectOptions(), ['prompt' => '请选择'])";
     ...
     * @return string
     */
    public function generateSearchField($tableSchema, $attribute)
    {
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            return null;
        }
        ...
        if ($column->phpType === 'boolean') {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "Html::activeDropDownList(\$model, '$attribute'," . $this->getModelClass() . "::get" . Inflector::pluralize(Inflector::id2camel(str_replace('_', '-', $column->name))) . "(), ['class' => 'form-control','prompt' => \$model->getAttributeLabel('$attribute')])";
            } else {
                return "Html::activeTextInput(\$model, '$attribute',['class' => 'form-control','placeholder'=>\$model->getAttributeLabel('$attribute')])";
            }
        }
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            $modelClassName = $this->generateClassName(str_replace('_', '-', $tableName));
            $controllerClassName = $this->generateClassName(str_replace('_', '-', $tableName)).'Controller';
            ...
            $params = [
                ...
                'controllerClassName' => $modelClassName,
                'queryClassName' => $controllerClassName,
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->controllerNs)) . '/' . $controllerClassName . '.php',
                $this->render('controller.php', $params)
            );

            $viewPath =dirname( Yii::getAlias('@' . str_replace('\\', '/', $this->controllerNs))).'/views/'.Inflector::camel2id($modelClassName);
    ...
    public function generateLabels($table)
    {
        ...
        foreach ($table->columns as $column) {
            if (!empty($column->comment)) {
                if (strpos($column->comment, '=') > 0) {
                    $labels[$column->name] = substr($column->comment, 0, strpos($column->comment, '='));
                } else {
    ...
    {
        return Inflector::id2camel(str_replace('_', '-', $tableName));
