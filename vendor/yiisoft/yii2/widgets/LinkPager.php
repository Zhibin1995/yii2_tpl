...
class LinkPager extends Widget
{
    ...
    public $customLabel = '<div class="pagination-custom">共 {TotalPage} 页，当前第 {CurrentPage} 页，共 {TotalItem} 条，每页显示 {PageSize}</div>';
    /**
    ...
    public function run()
    {
        ...
        $parts['{TotalPage}'] = $this->pagination->getPageCount();
        $parts['{CurrentPage}'] = min($this->pagination->getPageCount(), $this->pagination->getPage() + 1);
        $parts['{TotalItem}'] = $this->pagination->totalCount;
        $parts['{PageSize}'] = Html::dropDownList('page-size', $this->pagination->getPageSize(), [10 => 10, 20 => 20, 50 => 50]);
        echo strtr($this->customLabel, $parts);
    }
